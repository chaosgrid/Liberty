#include "Content-PCH.h"

CLANG_FORWARD_PROC_SYMBOL(public_6eb7260);
CLANG_FORWARD_PROC_SYMBOL(public_6efff70);
CLANG_FORWARD_PROC_SYMBOL(public_6fa912a);
CLANG_FORWARD_JUMP_SYMBOL(public_6facab3);

PROC_DECLARE(0x6efff70, internal_6efff70, public_6efff70);
extern "C" NAKED register_t __cdecl internal_6efff70()
{
    __asm
    {
        push 0xFFFFFFFF
/*FIXUP push public_6facab3 @0x6efff72*/
  FIXUP push esp
  FIXUP push esp
  FIXUP mov dword ptr ds : [esp], eax
  FIXUP mov eax, public_6facab3
  FIXUP mov dword ptr ds : [esp+4], eax
  FIXUP pop eax
        mov eax, dword ptr fs : [0]
        push eax
        mov dword ptr fs : [0], esp
        sub esp, 8
        push esi
        mov esi, ecx
        push edi
        mov dword ptr ss : [esp+0xC], esi
        call public_6eb7260
        xor edi, edi
        or eax, 0xFFFFFFFF
        mov dword ptr ds : [esi+0x48], eax
        mov dword ptr ds : [esi+0x4C], eax
        mov al, byte ptr ss : [esp+0xB]
        mov dword ptr ds : [esi+0x44], edi
        mov dword ptr ss : [esp+0x18], edi
        mov byte ptr ds : [esi+0x50], al
        mov dword ptr ds : [esi+0x54], edi
        mov dword ptr ds : [esi+0x58], edi
        mov dword ptr ds : [esi+0x5C], edi
        mov cl, byte ptr ss : [esp+0xB]
        push 0xC
        mov byte ptr ss : [esp+0x1C], 1
        mov byte ptr ds : [esi+0x60], cl
        call public_6fa912a
        mov ecx, dword ptr ss : [esp+0x14]
        mov dword ptr ds : [eax], eax
        mov dword ptr ds : [eax+4], eax
        mov dword ptr ds : [esi+0x64], eax
        mov dword ptr ds : [esi+0x68], edi
        add esp, 4
        mov dword ptr ds : [esi+0x6C], edi
        mov dword ptr ds : [esi+0x70], edi
        mov dword ptr ds : [esi+0x74], edi
        pop edi
        mov eax, esi
        pop esi
        mov dword ptr fs : [0], ecx
        add esp, 0x14
        ret 
        UNREACHABLE_TRAP(0x6efff70)
    }
}
